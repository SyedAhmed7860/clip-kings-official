document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    // 4. PAGE SPECIFIC SCRIPTS (Runs on Barba Enter)
    // ==========================================
    function initPageScripts(container) {
        
        // Active Nav Highlighting globally (Desktop & Mobile)
        ...
        const videoModal = container.querySelector('#video-modal') || document.getElementById('video-modal');
        const modalVideo = container.querySelector('#modal-video') || document.getElementById('modal-video');
        const closeModalBtn = container.querySelector('#close-modal') || document.getElementById('close-modal');
    ...
    if (typeof barba !== 'undefined') {
        barba.init({
            preventRunning: true,
            transitions: [{
                name: 'premium-barber-transition',
                leave(data) {
                    const tl = gsap.timeline();
                    tl.to('.slice-left, .slice-right', {
                        scaleX: 1,
                        duration: 0.6,
                        ease: "power4.inOut"
                    })
                ...
                enter(data) {
                    lenis.start(); // Ensure scrolling is never locked on page transition
